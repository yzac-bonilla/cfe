...
<?php
  if ($_SESSION['usuario']!="admin") 
  {
      ...
      <data-dismiss="alert"><strong>Acceso Denegado: </strong>Solo el usuario "admin" puede eliminar usuarios.
      </div>
  ...
  $result=mysqli_query($conexion,"SELECT * FROM usuarios ORDER BY Id DESC");
    ...
    if($row["Usuario"]!="admin") 
    {
	    ...
	    <tr>
	      <th><?=$row["Id"]?></th>
        <?php
        printf("<td bgcolor='#ffbfbf'><a href='Eliminar_Usuario.php?id=%d' onclick=\"return confirmar('¿Está seguro que desea eliminar el registro?')\">Borrar</a></td>",$row["Id"]);
        ?>
	      <td><?=$row["Usuario"]?></td>
	      <td><?=$row["Permisos"]?></td>
	    </tr>
