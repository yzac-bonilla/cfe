<?php
  ...
  $buscar_1 = $_POST["seleccion_1"];
  $buscar_2 = $_POST["seleccion_2"];
  if ($buscar_1=="Default" && $buscar_2=="Default") 
  {
  ...
  {
    $buscar_x="Entidad_Federativa like '%".$buscar_1."%'";
    $color_x="#cc0000";
  }
  if ($buscar_2=="Default") 
  {
  ...
  {
    $buscar_y="Division like '%".$buscar_2."%'";
    $color_y="#cc0000";
  }
?>
...
<div class="container">
  ...
  <div class="btn-group">
    ...
    <a href="Consulta_PGL.php" class="btn btn-info">Ver Todo</a></button>
  </div>
  <br><br>
 ...
 <div class="col-md-6" align="right">
  ...
  <?php     
    $consulta_mysql= mysqli_query ($conexion,"SELECT * FROM pgl WHERE $buscar_x $aux $buscar_y");
  ?>
...
<section>
  ...
  while($row=mysqli_fetch_array($consulta_mysql))
  {
    ...
    <tr>
      ...
      <td><font color="<?=$color_x?>"><b><?=$row["Entidad_Federativa"]?></b></font></td>
      <td><font color="<?=$color_y?>"><b><?=$row["Division"]?></b></td>
      <td><b><?=$row["Obras"]?></b></td>
      <td><?=$row["SE"]?></td>
      <td><?=$row["LT"]?></td>
  ...
  <tr>
      ...
      <td colspan="3"><strong><?=$contador?> Proyecto(s)</strong></td>
      <?php  
        $result = mysqli_query($conexion,"SELECT SUM(SE) AS sum FROM pgl WHERE $buscar_x $aux $buscar_y"); 
        $row = mysqli_fetch_assoc($result); 
      ?>
      <td><strong><?=$row['sum']?></strong></td>
      <?php  
        $result = mysqli_query($conexion,"SELECT SUM(LT) AS sum FROM pgl WHERE $buscar_x $aux $buscar_y"); 
        $row = mysqli_fetch_assoc($result); 
      ?>
      <td><strong><?=$row['sum']?></strong></td>
      <?php  
        $result = mysqli_query($conexion,"SELECT SUM(Alimentadores_AT) AS sum FROM pgl WHERE $buscar_x $aux $buscar_y"); 
        $row = mysqli_fetch_assoc($result); 
      ?>
      <td><strong><?=$row['sum']?></strong></td>
      <?php  
        $result = mysqli_query($conexion,"SELECT SUM(Alimentadores_MT) AS sum FROM pgl WHERE $buscar_x $aux $buscar_y"); 
        $row = mysqli_fetch_assoc($result); 
      ?>
      <td><strong><?=$row['sum']?></strong></td>
      <?php  
        $result = mysqli_query($conexion,"SELECT SUM(MVA) AS sum FROM pgl WHERE $buscar_x $aux $buscar_y"); 
        $row = mysqli_fetch_assoc($result); 
      ?>
      <td><strong><?=$row['sum']?></strong></td>
      <?php  
        $result = mysqli_query($conexion,"SELECT SUM(MVAr) AS sum FROM pgl WHERE $buscar_x $aux $buscar_y"); 
        $row = mysqli_fetch_assoc($result); 
      ?>
      <td><strong><?=$row['sum']?></strong></td>
      <?php  
        $result = mysqli_query($conexion,"SELECT SUM(KmC) AS sum FROM pgl WHERE $buscar_x $aux $buscar_y"); 
        $row = mysqli_fetch_assoc($result); 
      ?>
      <td><strong><?=$row['sum']?></strong></td>
      <?php  
        $result = mysqli_query($conexion,"SELECT SUM(Numero_Aliment) AS sum FROM pgl WHERE $buscar_x $aux $buscar_y"); 
        $row = mysqli_fetch_assoc($result); 
      ?>
      <td><strong><?=$row['sum']?></strong></td>
      <?php  
        $result = mysqli_query($conexion,"SELECT SUM(Medidores) AS sum FROM pgl WHERE $buscar_x $aux $buscar_y"); 
        $row = mysqli_fetch_assoc($result); 
      ?>
      <td><strong><?=$row['sum']?></strong></td>
      <?php  
        $result = mysqli_query($conexion,"SELECT SUM(Transformadores_Distribucion) AS sum FROM pgl WHERE $buscar_x $aux $buscar_y"); 
        $row = mysqli_fetch_assoc($result); 
      ?>
      <td><strong><?=$row['sum']?></strong></td>
      <?php  
        $result = mysqli_query($conexion,"SELECT SUM(Postes) AS sum FROM pgl WHERE $buscar_x $aux $buscar_y"); 
        $row = mysqli_fetch_assoc($result); 
      ?>
      <td><strong><?=$row['sum']?></strong></td>
      <td colspan="14"></td>
      <?php  
        $result = mysqli_query($conexion,"SELECT SUM(Act_Prev_Inv) AS sum FROM pgl WHERE $buscar_x $aux $buscar_y"); 
        $row = mysqli_fetch_assoc($result); 
      ?>
      <td><strong>$<?php echo number_format($row['sum'], 2, '.', ','); ?></strong></td>
      <?php  
        $result = mysqli_query($conexion,"SELECT SUM(Monto_Max_Publicado) AS sum FROM pgl WHERE $buscar_x $aux $buscar_y"); 
        $row = mysqli_fetch_assoc($result); 
      ?>
      <td><strong>$<?php echo number_format($row['sum'], 2, '.', ','); ?></strong></td>
      <?php  
        $result = mysqli_query($conexion,"SELECT SUM(Monto_Adjudicado) AS sum FROM pgl WHERE $buscar_x $aux $buscar_y"); 
        $row = mysqli_fetch_assoc($result);
