...
<div class="container">
  ...
  <tr>
    <?php  
        $result = mysqli_query($conexion,"SELECT count(Total_SAOs) AS aux FROM $base_datos");
        $row = mysqli_fetch_assoc($result);
    ?>
    <td width="100" align="middle"><b>Total de SAO's = <?=$row['aux']?></b></td>
    <?php  
        $result = mysqli_query($conexion,"SELECT sum(Oficios_Atendidos) AS aux FROM $base_datos WHERE Status='ATENDIDO'"); 
        $row = mysqli_fetch_assoc($result);
    ?>
    <td width="100" align="middle"><b>Oficios Atendidos = <?=$row['aux']?></b></td>
    <?php  
        $result = mysqli_query($conexion,"SELECT Round(avg(Tiempo_Respuesta)) AS aux FROM $base_datos"); 
        $row = mysqli_fetch_assoc($result);
        mysqli_free_result($filtro);
        mysqli_free_result($result);
        mysqli_close($conexion);
    ?>
    <td width="100" align="middle"><b>Tiempo Promedio = <?=$row['aux']?> dias</b></td>
  </tr>
  </table>
  </font>
