 <?php
  ...
  {
    $buscar_x="Familia like '%".$buscar_1."%'";
    $color_x="#cc0000";
  }
  if ($buscar_2=="Default") 
  {
  ...
  {
    $buscar_y="Empresa like '%".$buscar_2."%'";
    $color_y="#cc0000";
  }
?>
...
<div class="container">
 ...
 <div class="btn-group">
  ...
  <?php     
  $consulta_mysql= mysqli_query ($conexion,"SELECT * FROM $base_datos WHERE $buscar_x $aux $buscar_y");
  ?>
     ...
     while($row = mysqli_fetch_assoc($consulta_mysql)) 
     {
      ?> 
        <tr>
          ...
          <td><?=$row["Sello"]?></td>
          <td><font color="<?=$color_y?>"><b><?=$row["Empresa"]?></b></font></td>
          <td><?=$row["Remitente"]?></td>
          <td><font color="<?=$color_x?>"><b><?=$row["Familia"]?></b></font></td>
