...
?>
  ...
  <form action="Upload_PDF_CN.php?id=<?=$año?>" method="post" enctype="multipart/form-data">
    <select class="form-control" name="oficio" autofocus>
      <option value="NO ESPECIFICADO" selected>Seleccionar registro</option>
      <?php
      $filtro=mysqli_query($conexion,"SELECT * FROM $base_datos ORDER BY Id DESC");
      while($fila=mysqli_fetch_array($filtro))
      {
          ?>
          <option value="<?=$fila['Oficio']?>"><?=$fila["Id"]?>.- <?=$fila["Empresa"]?> / <?=$fila["Familia"]?></option>
          <?php
      }
    ...
    <br><input type="file" class="form-control" accept=".pdf" name="fileToUpload" id="fileToUpload">
    <br><center><input type="submit" value="Subir PDF" name="submit" class="btn btn-primary">
